...
$root_path = dirname(dirname(__FILE__)).'/wp-content/uploads';

// Root url for links in file manager.Relative to $http_host. Variants: '', 'path/to/subfolder'
// Will not working if $root_path will be outside of server document root
$root_url = '/wp-content/uploads';
...
if (isset($_GET['del']) && !FM_READONLY) {
    ...
    wp_redirect(FM_SELF_URL . '?p=' . urlencode(FM_PATH));
}
...
if (isset($_GET['upload']) && !FM_READONLY) {
    ...
    <div class="container mt-3">
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <br>
            <p>
                <button type="submit" class="btn btn-primary"><i class="las la-upload"></i> Upload</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>" class="text-secondary"><i class="las la-times-circle"></i> Cancel</a></b>
            </p>
        </form>
    </div>
    <?php
...
<!--                <label><input type="checkbox" name="move" value="1"> Move</label>-->
            ...
            <hr>
            <p>
                <button type="submit" class="btn btn-primary"><i class="las la-copy"></i> Copy </button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>" class="text-secondary"><i class="las la-times-circle"></i> Cancel</a></b>
            </p>
        </form>
    </div>
...
if (isset($_GET['copy']) && !isset($_GET['finish']) && !FM_READONLY) {
    ...
    <div class="container mt-3">
        ...
        <p>
            <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1"><i class="las la-check-circle"></i> Copy</a></b> &nbsp;
            <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode($copy) ?>&amp;finish=1&amp;move=1"><i class="las la-check-circle"></i> Move</a></b> &nbsp;
            <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="las la-times-circle"></i> Cancel</a></b>
        </p>
        ...
        <ul class="folders break-word">
            ...
            if ($parent !== false) {
                ?>
                <li><a href="?p=<?php echo urlencode($parent) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="las la-chevron-circle-left"></i> ..</a></li>
                <?php
            }
            foreach ($folders as $f) {
                ?>
                <li><a href="?p=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>&amp;copy=<?php echo urlencode($copy) ?>"><i class="las la-folder-o"></i> <?php echo fm_convert_win($f) ?></a></li>
                <?php
            }
            ?>
        </ul>
...
if (isset($_GET['view'])) {
    ...
    <div class="path pt-3">
        ...
        <p>
            <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($file) ?>"><i class="las la-cloud-download"></i> Download</a></b> &nbsp;
            <b><a class="btn-outline-primary btn-sm btn" href="<?php echo fm_enc($file_url) ?>" target="_blank"><i class="las la-external-link-square"></i> Open</a></b> &nbsp;
            <?php
            // ZIP actions
            if (!FM_READONLY && $is_zip && $filenames !== false) {
                $zip_name = pathinfo($file_path, PATHINFO_FILENAME);
                ?>
                <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>"><i class="las la-check-circle"></i> UnZip</a></b> &nbsp;
                <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;unzip=<?php echo urlencode($file) ?>&amp;tofolder=1" title="UnZip to <?php echo fm_enc($zip_name) ?>"><i class="las la-check-circle"></i>
                        UnZip to folder</a></b> &nbsp;
                <?php
            }
            if($is_text && !FM_READONLY) {
                ?>
                <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>" class="edit-file"><i class="las la-pencil-square"></i> Edit</a></b> &nbsp;
                <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&env=ace" class="edit-file"><i class="las la-pencil-square"></i> Advanced Edit</a></b> &nbsp;
            <?php 
            }
            ?>
            <b><a class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(FM_PATH) ?>"><i class="las la-chevron-circle-left"></i> Back</a></b>
        </p>
        <?php
        if ($is_zip) {
            // ZIP content
...
if (isset($_GET['edit'])) {
    ...
    <div class="path pt-3">
        <div class="mb-3">
            <a title="Cancel" class="btn-outline-primary btn-sm btn" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;view=<?php echo urlencode($file) ?>"><i class="las la-reply-all"></i> Cancel</a>
            <a title="Backup" class="btn-outline-primary btn-sm btn" href="javascript:backup('<?php echo urlencode($path) ?>','<?php echo urlencode($file) ?>')"><i class="las la-database"></i> Backup</a>
            <?php if($is_text) { ?>
                <?php if($isNormalEditor) { ?>
                    <a class="btn-outline-primary btn-sm btn" title="Advanced" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>&amp;env=ace"><i class="las la-paper-plane"></i> Advanced Editor</a>
                    <button class="btn-outline-primary btn-sm btn" type="button" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'nrl')"><i class="las la-floppy-o"></i> Save</button>
                <?php } else { ?>
                    <a class="btn-outline-primary btn-sm btn" title="Plain Editor" href="?p=<?php echo urlencode(trim(FM_PATH)) ?>&amp;edit=<?php echo urlencode($file) ?>"><i class="las la-text-height"></i> Plain Editor</a>
                    <button class="btn-outline-primary btn-sm btn" type="button" name="Save" data-url="<?php echo fm_enc($file_url) ?>" onclick="edit_save(this,'ace')"><i class="las la-floppy-o"></i> Save</button>
                <?php } ?>
            <?php } ?>
        </div>
        <?php
...
if (isset($_GET['chmod']) && !FM_READONLY && !FM_IS_WIN) {
    ...
    <div class="py-3 container">
        ...
        <form action="" method="post">
            ...
            <p>
                <button type="submit" class="btn btn-primary"><i class="las la-check-circle"></i> Change</button> &nbsp;
                <b><a href="?p=<?php echo urlencode(FM_PATH) ?>" class="text-secondary"><i class="las la-times-circle"></i> Cancel</a></b>
            </p>
...
<form class="mt-3 mx-3" action="" method="post">
    ...
    <table class="table" id="main-table"><thead><tr>
        ...
        if ($parent !== false) {
            ?>
            <tr><?php if (!FM_READONLY): ?><td></td><?php endif; ?><td colspan="<?php echo !FM_IS_WIN ? '6' : '4' ?>"><a href="?p=<?php echo urlencode($parent) ?>"><i class="las la-chevron-circle-left"></i> ..</a></td></tr>
            <?php
        }
        foreach ($folders as $f) {
            ...
            <tr>
                ...
                <td class="inline-actions"><?php if (!FM_READONLY): ?>
                        <a title="Delete" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>"
                           onclick="return confirm('Delete folder?');"><i class="las la-trash" aria-hidden="true"></i></a>
                        <a title="Rename" href="#"
                           onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i
                                class="las la-edit" aria-hidden="true"></i></a>
                        <a title="Copy to..."
                           href="?p=&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i
                                class="las la-copy" aria-hidden="true"></i></a>
                    <?php endif; ?>
                    <a title="Direct link"
                       href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f . '/') ?>"
                       target="_blank"><i class="las la-link" aria-hidden="true"></i></a>
                </td>
            </tr>
            <?php
            flush();
        ...
        foreach ($files as $f) {
            ...
            <tr>
                ...
                <td class="inline-actions">
                    <?php if (!FM_READONLY): ?>
                        <a title="Delete" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;del=<?php echo urlencode($f) ?>" onclick="return confirm('Delete file?');"><i class="las la-trash"></i></a>
                        <a title="Rename" href="#" onclick="rename('<?php echo fm_enc(FM_PATH) ?>', '<?php echo fm_enc($f) ?>');return false;"><i class="las la-edit"></i></a>
                        <a title="Copy to..." href="?p=<?php echo urlencode(FM_PATH) ?>&amp;copy=<?php echo urlencode(trim(FM_PATH . '/' . $f, '/')) ?>"><i class="las la-copy"></i></a>
                    <?php endif; ?>
                    <a title="Direct link" href="<?php echo fm_enc(FM_ROOT_URL . (FM_PATH != '' ? '/' . FM_PATH : '') . '/' . $f) ?>" target="_blank"><i class="las la-link"></i></a>
                    <a title="Download" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;dl=<?php echo urlencode($f) ?>"><i class="las la-download"></i></a>
                </td></tr>
            <?php
            flush();
        }
    ...
    <?php if (!FM_READONLY): ?>
        <div class="mb-5 row">
            <div class="col-sm-9 offset-sm-3"><a href="#/select-all" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary" onclick="select_all();return false;"><i class="las la-check-square"></i> Select all</a> &nbsp;
            <a href="#/unselect-all" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary" onclick="unselect_all();return false;"><i class="las la-window-close"></i> Unselect all</a> &nbsp;
            <a href="#/invert-all" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary" onclick="invert_all();return false;"><i class="las la-th-list"></i> Invert selection</a> &nbsp;
            ...
            <a href="javascript:document.getElementById('a-delete').click();" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary"><i class="las la-trash"></i> Delete </a> &nbsp;
            <input type="submit" class="hidden" name="zip" id="a-zip" value="Zip" onclick="return confirm('Create archive?')">
            <a href="javascript:document.getElementById('a-zip').click();" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary"><i class="las la-file-archive-o"></i> Zip </a> &nbsp;
            <input type="submit" class="hidden" name="copy" id="a-copy" value="Copy">
            <a href="javascript:document.getElementById('a-copy').click();" class="mt-2 mt-sm-0 btn2 btn btn-small btn-outline-primary"><i class="las la-copy"></i> Copy </a>
            </div>
        </div>
    <?php endif; ?>
</form>
...
function fm_show_nav_path($path)
{
    ...
    <nav class="navbar navbar-light bg-light navbar-expand-lg" style="background: linear-gradient(to left, #99ccff 0%, #ccffcc 100%);">
        ...
        $sep = '<i class="las la-caret-right text-secondary"></i>';
        if ($path != '') {
        ...
        <div class="navbar-nav float-rightd ml-auto">
            <?php if (!FM_READONLY): ?>
                <li class="nav-item"><a class="nav-link mx-1" title="Search" href="javascript:showSearch('<?php echo urlencode(FM_PATH) ?>')"><i class="las la-search fa-fw"></i> Search</a></li>
            <li class="nav-item"><a class="nav-link mx-1" title="Upload files" href="?p=<?php echo urlencode(FM_PATH) ?>&amp;upload"><i class="las la-cloud-upload fa-fw" aria-hidden="true"></i> Upload Files</a></li>
                <li class="nav-item"><span class="nav-link mx-1" title="New folder" href="#" style="outline: none;" data-toggle="modal" onclick="document.getElementById('createNewItem').style.display='block';"><i class="las la-plus-square fa-fw"></i> New Item</span></li>
            <?php endif; ?>
        </div>
        </div>
    </nav>
...
?>
    ...
    <style>
        ...
        .close, .close:focus, .close:hover, a {
...
<div id="wrapper">
    ...
    <div class="modal" tabindex="-1" role="dialog" id="createNewItem" style="display:none;">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="las la-plus-square fa-fw"></i> Create New Item</h5>
                ...
                <div class="modal-footer">
                    <button type="submit" name="submit" value="Create Now" onclick="newfolder('<?php echo fm_enc(FM_PATH) ?>');return false;" class="btn btn-success"><i class="las la-plus-square fa-fw"></i> Create Now</button>
...
<!--    </div>-->
    ...
    <div class="modal" tabindex="-1" role="dialog" id="searchResult">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title"><i class="las la-search fa-fw"></i> Search Files and Folders</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
...
<script>
    ...
    function showSearch(e) {
        ...
        t.open("POST", "", !0), t.setRequestHeader("Content-type", "application/x-www-form-urlencoded"), t.onreadystatechange = function () {
            4 == t.readyState && 200 == t.status && (window.searchObj = t.responseText, document.getElementById("searchresultWrapper").innerHTML = "", window.location.hash = "#searchResult",document.getElementById('searchResult').style.display='block')
        }, t.send(n)
    }
