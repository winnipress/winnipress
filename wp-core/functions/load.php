...
function wp_set_lang_dir(){
	if(!defined('WP_LANG_DIR')){
		if(file_exists(WP_CONTENT_DIR . '/languages') && @is_dir(WP_CONTENT_DIR . '/languages') || !@is_dir(ABSPATH . 'wp-core/languages')){
			/**
			 ...
			 * @since 2.1.0
			 */
			define('WP_LANG_DIR', ABSPATH . 'wp-core/languages');
			if(!defined('LANGDIR')){
				// Old relative path maintained for backward compatibility.
				define('LANGDIR', 'wp-core/languages');
			}
		}
	}
}
...
function require_wp_db(){
	...
	require_once(ABSPATH . 'wp-core/wp-db.php');
...
function wp_set_wpdb_vars(){
	...
	$wpdb->field_types = array('post_author' => '%d', 'post_parent' => '%d', 'term_id' => '%d', 'term_group' => '%d', 'term_taxonomy_id' => '%d',
		'parent' => '%d', 'count' => '%d','object_id' => '%d', 'term_order' => '%d', 'ID' => '%d', 'comment_ID' => '%d',
		'user_id' => '%d', 'option_id' => '%d', 'blog_id' => '%d', 'meta_id' => '%d', 'post_id' => '%d',
...
function wp_start_object_cache(){
	...
	if(!wp_using_ext_object_cache()){
		require_once (ABSPATH . 'wp-core/cache.php');
	}
...
function wp_load_translations_early(){
	...
	require_once ABSPATH . 'wp-core/pomo/mo.php';
	...
	while (true){
		...
		if(@is_dir(ABSPATH . 'wp-core/languages'))
			$locations[] = ABSPATH . 'wp-core/languages';
